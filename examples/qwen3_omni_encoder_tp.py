...
Usage (single-host, 8-GPU layout):
    ...
    # Same PR-branch command shape for a controlled tp=1 baseline:
    python examples/qwen3_omni_encoder_tp.py \\
        --model Qwen/Qwen3-Omni-30B-A3B-Instruct \\
...
GPU as in the default speech config. The default Qwen3-Omni config now
uses ``backend="auto"``, which resolves to the SGLang encoder runner for
supported Qwen3 encoders; this script pins ``"sglang"`` explicitly for
deployment clarity.
"""
...
def _apply_ar_mem_fraction(stage, fraction: float | None):
    """Set the typed resident/static AR memory budget for one SGLang stage."""
    ...
    runtime = stage.runtime.model_copy(
        update={
            "resources": stage.runtime.resources.model_copy(
                update={"total_gpu_memory_fraction": fraction}
            ),
...
def main() -> None:
    ...
    for s in cfg.stages:
        if s.name == "preprocessing" and args.thinker_max_seq_len is not None:
            ...
            s = s.model_copy(
                update={
                    "factory_args": {**s.factory_args, "backend": args.encoder_backend},
