# Encoder TP Parity Findings (Plan B)
...
Current workspace note (2026-05-26): CUDA is accessible in this workspace
(`torch.cuda.device_count()==8`, NVIDIA H200). The current evidence directory
is `/data/encoder_tp_evidence_20260526`. GPU memory is shared with processes
outside this namespace, so startup/E2E runs use explicit AR
`mem_fraction_static=0.45` to keep thinker/talker KV allocation within
available headroom.

The RFC Phase 1 spec calls for `atol=1e-3, rtol=1e-3` on fp16. The results
below show what the implementations actually produce. Image and video fail the
strict RFC tolerance because HF and SGLang use different vision implementations
and because TP changes fp16 reduction order; audio is strict-allclose after the
adapter flattening fix.
...
| image: sglang wrapper (tp=1) vs sglang wrapper (tp=2) | 0.208 | 0.0008 | 0.999982 | ❌ (fp16 TP tail) |
| video: sglang wrapper (tp=1) vs sglang wrapper (tp=2) | 0.238 | 0.0010 | 0.999980 | ❌ (fp16 TP tail) |
| audio: sglang wrapper (tp=1) vs sglang wrapper (tp=2) | 0.0004 | 0.0000 | 0.999999 | ✅ |
| image: local HF (tp=1) vs sglang wrapper (tp=1) | 5.953 | 0.0217 | 0.987705 | ❌ |
| video: local HF (tp=1) vs sglang wrapper (tp=1) | 3.719 | 0.0217 | 0.990721 | ❌ |
| audio: local HF (tp=1) vs sglang wrapper (tp=1) | 0.0003 | 0.0000 | 0.999999 | ✅ |
...
0.208 max-abs tail is NCCL fp16 accumulation-order noise and later
non-linear amplification; 97.5% of tokens have cos sim ≥ 0.9999. The
wrapper's TP plumbing is correct.

The 2026-05-26 video and audio lanes use the production preprocessor path:
`tests/data/draw.mp4` for video and `tests/data/query_to_cars.wav` for audio.
Video shows the same SGLang-implementation-vs-HF gap as image, while SGLang
TP remains tight. Audio is strict-allclose for both local-vs-SGLang and
TP1-vs-TP2 after fixing the SGLang audio adapter to flatten valid upstream
`[B, T, D]` audio output into `[sum(valid_lengths), D]`.
...
# The comparator now evaluates any present top-level `image_embeds`,
# `video_embeds`, and `audio_embeds` tensors plus visual deepstack lists. Use
# `--labels audio_embeds` or `--labels video_embeds` for focused modality gates
# once GPU runs have generated audio/video parity pickles.

# 5. Capture video parity through the same production preprocessor path
CUDA_VISIBLE_DEVICES=2 PYTHONPATH=. \
  python tests/_encoder_parity_harness.py local /tmp/parity_local_video.pkl \
    --modality video
CUDA_VISIBLE_DEVICES=2 PYTHONPATH=. \
  python tests/_encoder_parity_harness.py sglang /tmp/parity_sglang_tp1_video.pkl \
    --modality video
RANK0_GPU=2 RANK1_GPU=3 EXTRA_ARGS="--modality video" \
  bash tests/run_tp2_parity.sh /tmp/parity_sglang_tp2_video.pkl 29711
python tests/parity_compare.py /tmp/parity_local_video.pkl \
  /tmp/parity_sglang_tp1_video.pkl --labels video_embeds
python tests/parity_compare.py /tmp/parity_sglang_tp1_video.pkl \
  /tmp/parity_sglang_tp2_video.pkl --tp --labels video_embeds

# 6. Capture audio parity through the same production preprocessor path
CUDA_VISIBLE_DEVICES=2 PYTHONPATH=. \
  python tests/_encoder_parity_harness.py local /tmp/parity_local_audio.pkl \
    --modality audio
CUDA_VISIBLE_DEVICES=2 PYTHONPATH=. \
  python tests/_encoder_parity_harness.py sglang /tmp/parity_sglang_tp1_audio.pkl \
    --modality audio
RANK0_GPU=2 RANK1_GPU=3 EXTRA_ARGS="--modality audio" \
  bash tests/run_tp2_parity.sh /tmp/parity_sglang_tp2_audio.pkl 29712
python tests/parity_compare.py /tmp/parity_local_audio.pkl \
  /tmp/parity_sglang_tp1_audio.pkl --labels audio_embeds
python tests/parity_compare.py /tmp/parity_sglang_tp1_audio.pkl \
  /tmp/parity_sglang_tp2_audio.pkl --tp --labels audio_embeds
...
| `[0.9000, 0.9900)` | 1 | 0.0 |
| `[0.9900, 0.9990)` | 9 | 0.1 |
| `[0.9990, 0.9999)` | 143 | 2.4 |
| `[0.9999, 1.0001)` | 5889 | **97.5** |

Mean per-token cosine similarity: **0.999982**.

Deepstack layers (3 of them) are similarly tight: max abs diff 0.0117,
0.0801, 0.1606; mean cosine sim 0.999997, 0.999974, 0.999962.

### Interpretation

TP1 vs TP2 outputs are effectively identical: 99.8% of tokens have
cosine similarity ≥ 0.999 (97.5% ≥ 0.9999), mean 0.999982. The
post-merger fp16 tail, maxing at 0.208 in the latest run, is consistent
with NCCL fp16 all-reduce accumulation-order noise plus downstream
non-linear amplification, not a structural correctness issue.
...
To pin down *where* the 0.2–0.3-class max-abs-diff comes from, both tp=1 and
...
So the final max-abs-diff is structurally explained: fp16 NCCL
...
question outside TP scope and remains a follow-up.
...
(within 0.1% per bucket). NCCL's fp16 noise (~0.2 max abs from
...
1. **Loosen the Phase 1 numerical-parity tolerance.** The current
   ...
   - `tp_size=1` vs `tp_size>1`: `mean_per_token_cosine_sim ≥ 0.999`
     ...
     `tests/parity_compare.py --tp` and passes today at 0.999982.
